...
const TodayMaterialsCard = () => {
    ...
    return (
        <View style={[styles.card, {gap: vs(20), width: Platform.isPad? '49%' : '100%', padding: vs(20),}]}>
          ...
          <View style={{ marginBottom: Platform.isPad? vs(8) : s(8), }}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('Catalog')}>
              <Text style={[styles.link, {fontSize: Platform.isPad? vs(16) : vs(14)}]}>{translations[store.language].посмотретьвсе}</Text>
            </TouchableOpacity>
          ...
          <View style={{width: '100%', height: 'auto', rowGap: vs(26) }}>
            {docs?.map((doc, index) => (
              <View key={index} style={[styles.materialRow, { columnGap: vs(12)}]}>
                ...
                <Text style={[styles.index, {width: 'auto', fontSize: Platform.isPad ? vs(18) : vs(16)}]}>{index + 1}</Text>
                
                <View style={[styles.content, {rowGap: vs(16)}]}>
                  
                  <Text style={[styles.materialTitle, {fontSize: Platform.isPad ? vs(18) : vs(16), fontWeight: '500'}]}>{doc?.name}</Text>
                  ...
                  <TouchableOpacity onPress={() => openPdf(doc?.pdf)} style={{backgroundColor: '#e0f0ff', paddingHorizontal: vs(8), paddingVertical: vs(4), borderRadius: 6, alignSelf: 'flex-start'}}>
                    ...
                    <Text style={[styles.tagText, {fontSize: Platform.isPad ? vs(14) : vs(12)}]}>{doc?.subject}</Text>

    ...
    return (
        <View style={{width: '100%', height: 'auto', borderRadius: vs(30), backgroundColor: '#cc3184', padding: vs(20)}}>
            <View style={{flexDirection: isInline ? 'row' : 'column', alignItems: isInline ? 'center' : 'flex-start', gap: s(15)}}>
                <Image style={{ borderRadius: vs(30), width: imageWidth, height: Platform.isPad? vs(150) : s(150), resizeMode: 'center', margin: -vs(10)}} source={store.pfp?.second_image?.url? { uri: store.pfp.second_image.url } : require('../../../assets/aveduMenuGirl.png')} />

    ...
    return (
        <View style={{width: '100%', height: 'auto', columnGap: gap, rowGap: vs(15), flexWrap: 'wrap', flexDirection: 'row', justifyContent: 'space-between'}}>
            {subjects?.map((subject, index) => {
                ...
                return (
                    <TouchableOpacity activeOpacity={0.8} onPress={() => setSelectedId(subject?.id)} key={index} style={{width: subjectWidth, height: isTablet? vs(80) : s(80), borderRadius: 20, backgroundColor: isSelected ? '#6A5AE0' : '#EFEEFC', padding: vs(25), alignItems: 'center', flexDirection: 'row', gap: vs(15)}}>

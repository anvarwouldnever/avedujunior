...
import { useTasks } from '../Tasks/hooks/getTasks'
                    ...
                    <View style={{width: '100%', rowGap: vs(25)}} key={index}>
                        ...
                        <View style={{width: '100%', gap: vs(20), flexWrap: 'wrap', flexDirection: 'row'}}>
                            ...
                            {month?.tests?.map((topic: any, index: number) => {
                                ...
                                return (
                                    <TouchableOpacity onPress={() => navigation.navigate('PreGame', {name: name, topic: topic.name, id: topic?.id, tasksId: id })} style={{width: subjectWidth, height: vs(240), borderWidth: 2, borderColor: color.primary, backgroundColor: 'white', borderRadius: 20, paddingVertical: vs(15), paddingHorizontal: vs(20), justifyContent: 'space-between'}} key={index}>
                                        <View style={{ backgroundColor: color.primary, width: '50%', alignItems: 'center', borderRadius: vs(15) }}>

...
const RegisterButton = ({ progress, setProgress, hasEmptyFieldsChild, password, password2, hasEmptyFieldsParent, register, loading, success }) => {
    ...
    return (
        <TouchableOpacity onPress={() => handlePress()} disabled={loading || success} style={{ width: '100%', height: isTablet? vs(45) : s(45), backgroundColor: success ? '#59E956' : '#6A5AE0', opacity: isButtonDisabled ? 0.5 : 1, borderRadius: 15, justifyContent: 'center', alignItems: 'center' }}>
            { loading ?

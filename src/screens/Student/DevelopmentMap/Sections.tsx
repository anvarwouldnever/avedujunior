    ...
    return (
        <ScrollView horizontal bounces showsHorizontalScrollIndicator={false} contentContainerStyle={{ columnGap: vs(8) }} style={{ flexDirection: 'row', height: 'auto' }}>
            {sections.map(item => {
                ...
                return (
                    <TouchableOpacity key={item.label} onPress={() => setSection(item?.label)} style={{ backgroundColor: isActive ? '#6A5AE0' : '#EFEEFC', justifyContent: 'center', alignItems: 'center', padding: vs(12), borderRadius: vs(16) }}>
